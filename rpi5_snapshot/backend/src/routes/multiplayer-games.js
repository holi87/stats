...
router.patch('/multiplayer/games/:code', async (req, res, next) => {
  try {
    ...
    const { displayName, minPlayers, maxPlayers, showInQuickMenu, isActive } = req.body || {};
    const details = [];
    ...
    if (!hasDisplayName && !hasMinPlayers && !hasMaxPlayers && !hasShowInQuickMenu && !hasIsActive) {
      return next(
        validationError([
          {
            field: 'body',
            message:
              'must include at least one of: displayName, minPlayers, maxPlayers, showInQuickMenu, isActive',
          },
        ])
      );
    }
...
router.get('/multiplayer/games/:code/options', async (req, res, next) => {
  try {
    ...
    const options = await listMultiplayerGameOptions({ gameId: game.id });
