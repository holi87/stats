...
export function MultiplayerMatchEditPage() {
      ...
      <section>
        ...
        <div className="card">
          <EmptyState
            ...
            description="Ten widok obsługuje gry manualne, specjalny kalkulator, Pociągi i Terraformację Marsa."
            action={
              <Link
                className="button secondary"
                to={`/games/${match.game.code}/matches${location.search || ''}`}
  ...
  const initialPlayers = [...match.players].sort((a, b) => (a.place ?? 0) - (b.place ?? 0));
  ...
  const ticketToRidePlayers = isTicketToRide
    ? [...match.ticketToRide.playersDetails].sort((a, b) => (a.place ?? 0) - (b.place ?? 0))
    : [];
  ...
  const terraformingPlayers = isTerraformingMars
    ? [...match.terraformingMars.playersDetails].sort((a, b) => (a.place ?? 0) - (b.place ?? 0))
    : [];
  ...
  const customCalculatorPlayers = isCustomCalculator
    ? [...match.customCalculator.playersDetails].sort((a, b) => (a.place ?? 0) - (b.place ?? 0))
    : [];
