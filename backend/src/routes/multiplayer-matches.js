...
function normalizeOptionIdsInput({ optionId, optionIds, details, optionField = 'optionId' }) {
  ...
  if (optionId !== undefined) {
    if (typeof optionId !== 'string' || !isUuid(optionId)) {
      details.push({ field: optionField, message: 'must be a valid UUID' });
    } else {
      normalizedSingleOptionId = optionId;
    }
  }

  let normalizedOptionIds;
  if (optionIds !== undefined) {
    if (!Array.isArray(optionIds)) {
      details.push({ field: 'optionIds', message: 'must be an array of UUIDs' });
      ...
      optionIds.forEach((value, index) => {
        if (typeof value !== 'string' || !isUuid(value)) {
          details.push({ field: `optionIds[${index}]`, message: 'must be a valid UUID' });
          return;
        }
        if (!seen.has(value)) {
          seen.add(value);
      ...
      normalizedOptionIds = deduplicated;
    }
  }

  if (
    normalizedSingleOptionId &&
    normalizedOptionIds &&
    !normalizedOptionIds.includes(normalizedSingleOptionId)
  ) {
    details.push({
      field: 'optionIds',
      message: 'must include optionId when both optionId and optionIds are provided',
    });
  }

  if (normalizedOptionIds !== undefined) {
    return normalizedOptionIds;
  }
  if (normalizedSingleOptionId) {
    return [normalizedSingleOptionId];
  }
  return undefined;
}

async function resolveOptionsForGame({
  game,
  optionIds,
  optionField = 'optionIds',
  fallbackCodes = [],
  ...
  if (optionIds !== undefined) {
    for (const optionId of optionIds) {
      ...
      const option = await getMultiplayerGameOptionByIdForGame(game.id, optionId, { activeOnly: true });
      if (!option) {
  ...
  const seenIds = new Set(resolved.map((option) => option.id));
  for (const code of fallbackCodes) {
    if (!code) {
      continue;
    }
    // eslint-disable-next-line no-await-in-loop
    const option = await getMultiplayerGameOptionByCodeForGame(game.id, code, { activeOnly: true });
    if (!option) {
  ...
  if (effectiveExclusive && resolvedWithoutBase.length > 1) {
    throw validationError([
      {
        field: optionField,
        message: 'supports a single option for this game',
      },
    ]);
  }
...
router.post('/multiplayer/matches', async (req, res, next) => {
  try {
    const { gameId, playedOn, notes, players, ticketToRide, optionId, optionIds } = req.body || {};
    const details = [];
    ...
    const normalizedOptionIds = normalizeOptionIdsInput({
      optionId,
      optionIds,
      details,
    ...
    if (game.scoringType === 'TTR_CALCULATOR') {
      ...
      if (
        ...
        options.length > 0 &&
        !options.some((option) => option.code === variant.code)
      ) {
        return next(
          validationError([
            {
              field: normalizedOptionIds.length === 1 ? 'optionId' : 'optionIds',
    ...
    if (game.scoringType === 'TM_CALCULATOR' && !featureFlags.simpleTmMode) {
      if (ticketToRide !== undefined) {
        return next(validationError([{ field: 'ticketToRide', message: 'not applicable' }]));
      }
...
router.patch('/multiplayer/matches/:id', async (req, res, next) => {
  try {
    ...
    const { playedOn, notes, players, ticketToRide, optionId, optionIds } = req.body || {};
    const details = [];
    ...
    const game = {
      ...
      optionsExclusive:
        match.game_options_exclusive === null || match.game_options_exclusive === undefined
          ? true
          : match.game_options_exclusive === true,
    };
    ...
    if (game.scoringType === 'TTR_CALCULATOR') {
      ...
      if (normalizedPlayers || ticketToRide !== undefined || normalizedOptionIds !== undefined) {
        ...
        options = await resolveOptionsForGame({
          game,
          optionIds: normalizedOptionIds,
          optionField:
            normalizedOptionIds === undefined || normalizedOptionIds.length <= 1
              ? 'optionId'
              : 'optionIds',
          fallbackCodes: [variant.code],
          fallbackField: 'ticketToRide.variantId',
    ...
    if (error && error.code === 'MULTIPLAYER_MATCH_PLAYER_SET_LOCKED') {
      return next(
        validationError([
          {
            field: 'players',
