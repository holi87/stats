...
export function useMultiplayerGame(code: string | undefined) {
  const { request } = useApi();
  return useQuery({
    queryKey: code ? queryKeys.multiplayerGame(code) : ['multiplayerGame', 'missing'],
    queryFn: () => request<MultiplayerGame>(`/api/v1/multiplayer/games/${code}`),
    enabled: Boolean(code),
  });
}

export function useMultiplayerGameOptions(code: string | undefined, options?: { enabled?: boolean }) {
  const { request } = useApi();
  const enabled = Boolean(code) && (options?.enabled ?? true);
  return useQuery({
    queryKey: code ? queryKeys.multiplayerGameOptions(code) : ['multiplayerGameOptions', 'missing'],
    queryFn: () =>
      request<MultiplayerGameOptionListResponse>(`/api/v1/multiplayer/games/${code}/options`),
    enabled,
  });
}
