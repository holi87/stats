...
export function MultiplayerTerraformingMarsMatchForm({
  ...
  useEffect(() => {
    ...
    if (optionId && !selectableOptionIdSet.has(optionId)) {
      setOptionId('');
      return;
    }

    if (requiresSelectableOption && !optionId) {
      setOptionId(selectableGameOptions[0].id);
    }
  }, [requiresSelectableOption, selectableGameOptions, selectableOptionIdSet, optionId]);
  ...
  const podiumPreview = useMemo(() => {
    ...
    for (let index = 0; index < sorted.length; index += 1) {
      ...
      podiumLines.push(`${currentPlace}: ${name}`);
    }

    return podiumLines.length > 0 ? podiumLines : null;
