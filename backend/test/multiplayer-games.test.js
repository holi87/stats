...
test('POST /api/v1/multiplayer/games allows optionsExclusive=false', async (t) => {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required to run multiplayer games tests');
  }
  ...
  const address = server.address();
  const port = typeof address === 'object' && address ? address.port : 3000;
  const code = uniqueCode('multiopts');
  ...
  const createResponse = await fetch(`http://localhost:${port}/api/v1/multiplayer/games`, {
    ...
    body: JSON.stringify({
      code,
      displayName: `Multi Options ${code}`,
      minPlayers: 2,
      maxPlayers: 6,
  ...
  assert.equal(createResponse.status, 201);
  const created = await createResponse.json();
  assert.equal(created.optionsExclusive, false);
});
...
test('PATCH /api/v1/multiplayer/games/:code updates optionsExclusive mode', async (t) => {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required to run multiplayer games tests');
  }
  ...
  const address = server.address();
  const port = typeof address === 'object' && address ? address.port : 3000;
  const code = uniqueCode('optsmode');
  const pool = getPool();
  ...
  await pool.query(
    `INSERT INTO multiplayer_games (
     ...
     VALUES ($1, $2, 'MANUAL_POINTS', 2, 5, true, true)`,
    [code, `Mode ${code}`]
  );
  ...
  assert.equal(patchResponse.status, 200);
  const patched = await patchResponse.json();
  assert.equal(patched.optionsExclusive, false);
});
